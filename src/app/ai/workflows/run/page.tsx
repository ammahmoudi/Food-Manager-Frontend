...
const WorkflowPage = () => {
  ...
  const handleWorkflowSelectChange = (keys: Iterable<React.Key>) => {
	...
	const tempSelected: Workflow | undefined = workflows.find((workflow) => String(workflow.id) === temp_id);
	...
	//   setInputFields(tempSelected.inputs);
	  ...
	  console.warn(`Selected workflow with id ${temp_id} not found in the collection.`);
  ...
  selectedKeys={selectedWorkflow ? new Set([String(selectedWorkflow.id)]) : new Set()} // Ensure selectedKeys is a Set of strings
>
              {(workflow) => (
                ...
                ([inputName, inputType]) => (
                  <div className="mb-4" key={inputName}>
                    ...
                    {inputType.startsWith("image") ? (
                      ...
                      <Input
                        value={inputValues[nodeId]?.[inputName] || ""}
                        onChange={(e) =>
