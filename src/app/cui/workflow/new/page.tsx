...
// Define types for the inputs and nodes
interface InputType {
	type: string;
	// You can add more fields if necessary for your inputs
}
...
const WorkflowPage = () => {
	...
	const handleSelectNode = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		if (selectedNode) {
			...
			setSelectedNodes((prevNodes) => [
				...prevNodes,
				{ id: nodeId, name: selectedNode.name, type: selectedNode.type, inputs: [] }, // Initially empty input array
			]);
			...
			setAvailableNodes((prevNodes) => prevNodes.filter((node) => node.id !== nodeId));
	...
	const handleAddInput = () => {
		...
		setAvailableInputs((prevInputs) => prevInputs.filter((input) => input !== selectedInput));
	...
	const handleInputChange = (nodeId: string, inputName: string, value: string) => {
		setInputs((prevInputs) => ({
			...prevInputs,
			[nodeId]: {
				...prevInputs[nodeId],
	...
	const handleFinalSubmit = async () => {
		try {
			...
			let json_data;
			try {
				// Parse the jsonText to ensure it's valid JSON
			...
			const constructedInputs = Object.keys(inputs).reduce((acc, nodeId) => {
				acc[nodeId] = inputs[nodeId];
				return acc;
			...
			console.log("Submitting the following request body:", JSON.stringify(requestBody, null, 2));
