...
import { addFood, updateFood, deleteFood } from "@/app/berchi/services/berchiApi";
...
const FoodForm: FC<FoodFormProps> = ({
  ...
  const handleSave = async () => {
    ...
    const saveFoodPromise =
      isEditMode && initialData
        ? updateFood(initialData.id, foodData)
        : addFood(foodData);

    try {
      const savedFood = await toast.promise(saveFoodPromise, {
  ...
  const handleDeleteFood = async () => {
    if (initialData) {
      ...
      try {
        await toast.promise(deleteFoodPromise, {
